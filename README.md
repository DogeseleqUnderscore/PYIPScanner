...
- **Vendor Identification** - Optional MAC address vendor lookup
...
2. Run the scanner using Python:
```bash
python scanner.py
```
...
Scan the default range (192.168.0.0/24):
```bash
python scanner.py
```
...
python scanner.py --range 192.168.1.1-192.168.1.100

# Shorthand notation
python scanner.py --range 192.168.1.1-100
...
python scanner.py --ips 192.168.1.1,192.168.1.10,192.168.1.50
```
...
python scanner.py --file ip_list.txt
```
...
```bash
# Custom ports
python scanner.py --range 192.168.1.0/24 --ports 80,443,8080,3389

# Port ranges
python scanner.py --range 192.168.1.0/24 --port-range 1-1000,3389,8000-9000

# Skip port scanning
python scanner.py --range 192.168.1.0/24 --skip-ports
```
...
python scanner.py --range 192.168.1.0/24 --workers 500
```
...
```bash
# Ignore dead hosts
python scanner.py --range 192.168.1.0/24 --ignore-types dead

# Show only hosts with open ports
python scanner.py --range 192.168.1.0/24 --ignore-types dead,alive
```
...
```bash
# Export to CSV
python scanner.py --range 192.168.1.0/24 --export-csv results.csv
```
...
python scanner.py --range 192.168.1.0/24 --skip-vendor
```
...
- Triggers some antiviruses on some other devices so they think the network is not secure.
